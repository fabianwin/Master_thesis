...
if __name__ == "__main__":
    ...
    df = doge_df.loc[:,feature_list_big]
    df.fillna(method="ffill", inplace=True)
    df = df.dropna()
    df.info(verbose=True)
    depth = len(list(df.columns[1:])) # OHCL + indicators without Date
    ...
    train_df_nomalized = df_nomalized[:-test_window-lookback_window_size]
    test_df_nomalized = df_nomalized[-test_window-lookback_window_size:]
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=5, optimizer=Adam, batch_size=32, model="CNN", depth=depth, comment="doge All features")
    #train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker = 32, training_batch_size=200, visualize=False, EPISODES=200000)
    #test_multiprocessing(CustomEnv, CustomAgent, test_df, test_df_nomalized, num_worker = 10, visualize=False, test_episodes=1000, folder="/Users/fabianwinkelmann/Library/Mobile Documents/com~apple~CloudDocs/Master Thesis/Code/Crypto_Sentiment_RL_trader/7.Reinforcement_Learning/doge/2022_03_08_19_30_Crypto_trader", name="1492.19_Crypto_trader", comment="3 months")
