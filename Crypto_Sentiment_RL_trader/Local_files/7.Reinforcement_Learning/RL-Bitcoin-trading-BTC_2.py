...
class CustomAgent:
    ...
    def __init__(self, lookback_window_size=50, lr=0.00005, epochs=1, optimizer=Adam, batch_size=32, model="", depth=0, comment=""):
        ...
        # folder to save models
        path = "/Users/fabianwinkelmann/Library/Mobile Documents/com~apple~CloudDocs/Master Thesis/Code/Crypto_Sentiment_RL_trader/7.Reinforcement_Learning/Variable_files/"
...
if __name__ == "__main__":
    ...
    #train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker = 32, training_batch_size=150, visualize=False, EPISODES=300000)
    #test_multiprocessing(CustomEnv, CustomAgent, test_df, test_df_nomalized, num_worker = 16, visualize=False, test_episodes=1000, folder="/Users/fabianwinkelmann/Library/Mobile Documents/com~apple~CloudDocs/Master Thesis/Code/Crypto_Sentiment_RL_trader/7.Reinforcement_Learning/Variable_files/2022_03_04_09_22_Crypto_trader_BEST_FINANCE", name="1935.27_Crypto_trader", comment="BEST BTC Finance")
    ...
    df = btc_df.loc[:,feature_list_big]
    df.fillna(method="ffill", inplace=True)
    df = df.dropna()
    df.info(verbose=True)
    depth = len(list(df.columns[1:])) # OHCL + indicators without Date
