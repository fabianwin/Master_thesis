...
if __name__ == "__main__":
    ...
    df = xrp_df.loc[:,feature_list_big]
    df.fillna(method="ffill", inplace=True)
    df = df.dropna()
    df.info(verbose=True)
    depth = len(list(df.columns[1:])) # OHCL + indicators without Date
    ...
    train_df_nomalized = df_nomalized[:-test_window-lookback_window_size]
    test_df_nomalized = df_nomalized[-test_window-lookback_window_size:]
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=5, optimizer=Adam, batch_size=32, model="CNN", depth=depth, comment="XRP All features")
