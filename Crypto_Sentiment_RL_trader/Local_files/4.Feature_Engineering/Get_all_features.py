...
def get_features(symbol):
    ...
    #get news sentiment
    my_file = 'news_set_sentiment_'+symbol+".csv"
    news_data = pd.read_csv(os.path.join(my_path, my_file), parse_dates=date_cols, dayfirst=True)
    news_data = construct_sentiment_feature_set(news_data, symbol,"news")
    print(symbol," news feature completed")
    ...
    #get news sentiment
    my_file = 'scaled_news_sentiment_feature_set_'+symbol+".csv"
    my_scaled_file = 'news_sentiment_feature_set_'+symbol+".csv"
    news_data = pd.read_csv(os.path.join(my_path, my_file), parse_dates=date_cols, dayfirst=True)
    scaled_news_data = pd.read_csv(os.path.join(my_path, my_scaled_file), parse_dates=date_cols, dayfirst=True)
    ...
    #merge DataFrame
    final_df = pd.merge(ticker_data, news_data, how='left', on='date')
    ...
    scaled_final_df = pd.merge(scaled_ticker_data, scaled_news_data, how='left', on='date')
