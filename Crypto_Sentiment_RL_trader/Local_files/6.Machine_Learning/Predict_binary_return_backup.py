...
def LogReg_Pred(feature_list, coin, set, feature_df, predict_return_df):
    ...
    df = missing_values_table(df_not_imputed)
    print(df)

    #create test set which was not imputed
    #test set must be 20% --> 365
    print()
    feature_list_1 =  df.loc[df['Missing Values'] < 1000].index.values
    print(len(feature_list))
    print()
    print(len(feature_list_1))

    df_1 = feature_df.loc[:,feature_list]
    df_1.dropna(axis=0, how='any',inplace=True)
    print(df_1.shape)
    #rest can be used for training
    ...
    C_OPTIONS = list = np.logspace(-4,4,10)
